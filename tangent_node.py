...
# print("##########")
# neighbor_list = get_tangent_node([12,5],[6,5],[-np.pi/2, 0],[-np.pi/2, 3.1215953196166426])
# print(neighbor_list)
    
# print("##########")
    
def get_goal(point,centerPoint,angle_range, goal):
    margin = 0.01
    neighbor_list = []
    vector_point_to_point = np.array(goal) - np.array(centerPoint)
    angle = np.arctan2(vector_point_to_point[1],vector_point_to_point[0])
    r = 1
    l = distance(centerPoint,goal)
    h_sq = l**2 - r**2
    if h_sq < 0:
        h_sq = 0
    h = np.sqrt(h_sq)
    ...
    vertical_vect1 = np.array([r*np.cos(angle+theta), r*np.sin(angle+theta)])
    t1 = vertical_vect1 + np.array(centerPoint)   
    angle_t1 = np.arctan2(vertical_vect1[1], vertical_vect1[0])  
    if np.max(angle_range) - np.min(angle_range) < np.pi:
            if angle_t1 <= np.max(angle_range) +margin and angle_t1 >= np.min(angle_range)-margin:
                    neighbor_list.append([list(t1),list(goal)])
                    
    if np.max(angle_range) - np.min(angle_range) > np.pi:
            if angle_t1 >= np.max(angle_range)-margin and angle_t1 <= np.pi+margin or angle_t1 >= -np.pi-margin and angle_t1<= np.min(angle_range)+margin: 
                    neighbor_list.append([list(t1),list(goal)])

    vertical_vect2 = np.array([r*np.cos(angle-theta), r*np.sin(angle-theta)])
    t2 = vertical_vect2 + np.array(centerPoint)    
    angle_t2 = np.arctan2(vertical_vect2[1], vertical_vect2[0])   
    if np.max(angle_range) - np.min(angle_range) < np.pi:
            if angle_t2 <= np.max(angle_range)+margin and angle_t2 >= np.min(angle_range)-margin:
                    neighbor_list.append([list(t2),list(goal)])
                    
    if np.max(angle_range) - np.min(angle_range) > np.pi:
            if angle_t2 >= np.max(angle_range)-margin and angle_t2 <= np.pi+margin or angle_t2 >= -np.pi-margin and angle_t2 <= np.min(angle_range)+margin: 
                    neighbor_list.append([list(t2),list(goal)])                   
    return neighbor_list

def get_goal_from_start(centerPoint,goal,point):
    vector_center_to_point = np.array(point) - np.array(centerPoint)
    neighbor_list = []
    vector_point_to_point = np.array(goal) - np.array(centerPoint)
    angle = np.arctan2(vector_point_to_point[1],vector_point_to_point[0])
    r = 1
    ...
    vertical_vect1 = np.array([r*np.cos(angle+theta), r*np.sin(angle+theta)])
    t1 = vertical_vect1 + np.array(centerPoint)   
    angle1 = np.arccos(np.dot(vector_center_to_point,vertical_vect1))
    neighbor_list.append([list(t1),list(goal)]) 

    vertical_vect2 = np.array([r*np.cos(angle-theta), r*np.sin(angle-theta)])
    t2 = vertical_vect2 + np.array(centerPoint)   
    angle2 = np.arccos(np.dot(vector_center_to_point,vertical_vect2)) 
    neighbor_list.append([list(t2),list(goal)])          
    if angle1 < angle2:
        neighbor_list.reverse()                 
    return neighbor_list

def distance_sum(goal, single_obstacle):
    sum = 0
    for i in single_obstacle:
        sum = sum + distance(i,goal)
    return sum/len(single_obstacle)

     
def rearrange(goal,obstacle):   
    for single_obstacle in obstacle:
        single_obstacle.sort(key=lambda e:distance(goal,e))
    obstacle.sort(key=lambda e:distance_sum(goal,e))
    return obstacle

# print(rearrange([22,26],obstacle))
...
#get_tangent_node(point1,point2,angle_range1,angle_range2)   
# def get_goal_(centerPoint,angle_range, goal, goal_direction):
# print(get_goal_([0,0],[-np.pi/2,0], [5,5], np.pi/4))
...
# get_goal_from_start(start, start_direction, goal, goal_direction)
# print(get_goal_from_start_([0,0],np.pi/2, [5,5],3.5*np.pi/4))
...
# print("##########")
# neighbor_list = get_goal([0,0],[-np.pi, np.pi],[1,0])
# print(neighbor_list)
# print("##########")
                    
   
class tangent_node:
    def __init__(self,point,parent,goal,start):
        self.goal = goal[0:2]
        # self.goal_direction = goal[2]
        self.start = start
        self.r = 1
        self.parent = parent
        self.neighbor = []
        ...
        # print(obstacle)
        # obstacle = rearrange(goal,obstacle)

        obstacle_ = cp.deepcopy(obstacle)
        # obstacle_ = rearrange(goal,obstacle_)
        # print(obstacle)
        for i in obstacle_:
        ...
        if len(point) == 3:
            self.get_neighbor_start(point)
            self.cost = 0
        if len(point) == 2:
            self.point = point[1]
            self.transit = point[0]
            #add from 1-2
            if distance(self.goal,self.point) > 0.01 and distance(self.parent, self.goal) > 0.01:
    ...
    def get_neighbor_start(self,point):
        
        line_list = []
        theta = point[-1]
        position = point[0:2]
        self.point = position
        vect1 = [self.r*np.cos(theta+np.pi/2),self.r*np.sin(theta+np.pi/2)]
        c1 = np.array(position) + np.array(vect1)
        self.center = c1
        # self.neighbor.append([self.point, self.point])
        # print("###########")
        # print(self.neighbor)
        # print("###########")
        goal_line = get_goal_from_start(c1,self.goal,self.point)
        

        for i in self.obstacle_class_list:
            for j in range(0,len(i.single_obstacle)):
                if distance(c1,i.single_obstacle[j]) > 0.01:
                    neighbor_list1 = get_tangent_node(c1,i.single_obstacle[j],[-np.pi,np.pi],i.angle[j])
                    line_list = [*line_list, *neighbor_list1]
    ...
    def get_neighbor(self): 
        goal_line = get_goal(self.point,self.center,self.angle_range,self.goal)
        line_list = [] 
        # self.neighbor.append([self.point, self.point])
        for i in self.obstacle_class_list:
            for j in range(0,len(i.single_obstacle)):
    ...
    def get_center_and_angle_range(self):
        if self.point == self.goal:
            vect = np.array(self.point) - np.array(self.transit)
            vect = vect/np.linalg.norm(vect)
            vertical_vect = np.array([vect[1],-vect[0]])*self.r
            center = vertical_vect + np.array(self.goal)
            return [list(center), [-np.pi, np.pi]]

        
        if self.point != self.goal:
            for i in self.obstacle_class_list:
                for j in range(0,len(i.single_obstacle)):   
                    if distance(self.point, i.single_obstacle[j]) - self.r < 0.01:
                        return [i.single_obstacle[j], i.angle[j]]
                    
        if distance(self.start[0:2], self.point) < 0.01:
            theta = self.start[-1]
            position = self.start[0:2]
            self.point = position
            vect1 = [self.r*np.cos(theta+np.pi/2),self.r*np.sin(theta+np.pi/2)]
            c1 = np.array(position) + np.array(vect1)
            return [list(c1),[-np.pi, np.pi]]
            
                    
    def get_center_and_angle_range_of_parent(self):
        for i in self.obstacle_class_list:
            for j in range(0,len(i.single_obstacle)):   
                if distance(self.parent, i.single_obstacle[j]) - self.r < 0.01:
                    return [i.single_obstacle[j], i.angle[j]]
        theta = self.start[-1]
        position = self.start[0:2]
        #add from 1-2
        if distance(self.parent, position) <0.01:
            vect1 = [self.r*np.cos(theta+np.pi/2),self.r*np.sin(theta+np.pi/2)]
            c1 = np.array(position) + np.array(vect1)      
            return [c1, [-np.pi,np.pi]]
      
# t = tangent_node([0,0,0],None,[0,5])   
# print(t.neighbor)


# t = tangent_node([[13,0],[13,5]],None,[20,26])   
# print(t.neighbor)
...
start_pos = [0,0,np.pi/4] 
# parent = [14.667399999999999, 12.8464]
# point = [[14.667399999999999, 12.8464],[14.667399999999999, 12.8464]]
# goal_pos = [18,26,np.pi/2]
# obstacle = [[[6, 5], [12, 5], [12, 10], [6.1, 10]], [[15, 17], [20, 17.5], [19, 22], [15, 22]], [[15.9, 9.5], [18, 6], [19.6, 12], [17, 13], [15.2, 12]], [[2, 17], [6, 17], [6.7, 21], [5, 23], [1.9, 19]]]
# parent = [19.9762, 22.2169]
# point = [[19.9762, 22.2169], [19.9762, 22.2169]]
# # goal_pos = [0,26,np.pi/2]
# print(tangent_node_(point,parent,goal_pos,start_pos,obstacle).neighbor)   
